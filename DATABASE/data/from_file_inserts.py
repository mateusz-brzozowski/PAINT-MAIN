...
def get_english_words(file_read):
    ...
    with open(file_read, 'r') as fp:
        for line in fp.readlines():
            nr, word = line.split(',')
            text = f"INSERT INTO words(word_id, language_id, word_length, word_number, word) VALUES ({int(nr)+1000}, 1, 4, 0, '{word[:-1]}');\n"
            file+=text
    return file
    ...
    with codecs.open(file_read, 'r', 'utf-8') as fp:
        for line in fp.readlines():
            ...
            if (len(line[:-2]) == nr_letters and random.randint(0, 80)%80==0):
                text = f"INSERT INTO words(word_id, language_id, word_length, word_number, word) VALUES ({start}, 2, {nr_letters}, 0, '{unidecode(line[:-2])}');\n"
                file+=text
                start+=1
                i+=1
            if i>max_amount:
